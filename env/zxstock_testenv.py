...
dji = pd.read_csv("/mnt/share/Code/toyXiong/data/^DJI.csv")
...
data_1 = pd.read_csv('/mnt/share/Code/toyXiong/data/dow_jones_30_daily_price.csv')
...
class StockTestEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            ax.plot(self.asset_memory, color='tomato', label="ddpg")
            ax.plot(account_growth, color='steelblue', label="^DJI")
            ax.grid(linestyle='-', linewidth=0.1)
            ax.legend()
            plt.savefig('/mnt/share/Code/toyXiong/image/test.png'.format(iteration))
            plt.close()
            print("total_reward:{}".format(
                self.state[0] + sum(np.array(self.state[1:29])*np.array(self.state[29:])) - 10000))
