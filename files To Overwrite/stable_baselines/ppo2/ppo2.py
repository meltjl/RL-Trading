...
class PPO2(ActorCriticRLModel):
    ...
    def __init__(self, policy, env, seedy=None, gamma=0.99, n_steps=128, ent_coef=0.01, learning_rate=2.5e-4, vf_coef=0.5, max_grad_norm=0.5, lam=0.95, nminibatches=4, noptepochs=4, cliprange=0.2, cliprange_vf=None, verbose=0, tensorboard_log=None, _init_setup_model=True, policy_kwargs=None, full_tensorboard_log=False):

        super(PPO2, self).__init__(policy=policy, env=env, verbose=verbose,
                                   requires_vec_env=True, _init_setup_model=_init_setup_model, policy_kwargs=policy_kwargs)
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                act_model = self.policy(self.sess, self.observation_space, self.action_space, self.n_envs, 1,
                                        n_batch_step, reuse=False, seedy=self.seedy, **self.policy_kwargs)
                with tf.compat.v1.variable_scope("train_model", reuse=False,
                                                 custom_getter=tf_util.outer_scope_getter("train_model")):
                    train_model = self.policy(self.sess, self.observation_space, self.action_space,
                                              self.n_envs // self.nminibatches, self.n_steps, n_batch_train,
                                              reuse=True, seedy=self.seedy, **self.policy_kwargs)
                ...
                with tf.compat.v1.variable_scope("input_info", reuse=False):
                    ...
                    tf.compat.v1.summary.scalar('old_neglog_action_probabilty',
