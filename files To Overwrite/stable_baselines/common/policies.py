...
def nature_cnn(scaled_images, **kwargs):
    ...
    layer_2 = activ(conv(layer_1, 'c2', n_filters=64, filter_size=4,
                         stride=2, init_scale=np.sqrt(2), **kwargs))
...
def mlp_extractor(flat_observations, net_arch, act_fun):
    ...
    for idx, (pi_layer_size, vf_layer_size) in enumerate(zip_longest(policy_only_layers, value_only_layers)):
        if pi_layer_size is not None:
            ...
            latent_policy = act_fun(linear(latent_policy, "pi_fc{}".format(
                idx), pi_layer_size, init_scale=np.sqrt(2)))

        if vf_layer_size is not None:
            assert isinstance(
                vf_layer_size, int), "Error: net_arch[-1]['vf'] must only contain integers."
            latent_value = act_fun(linear(latent_value, "vf_fc{}".format(idx),
                                          vf_layer_size, init_scale=np.sqrt(2)))
...
class BasePolicy(ABC):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch,  reuse=False, scale=False, seed=None,
...
class ActorCriticPolicy(BasePolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, seedy=None, scale=False):
        super(ActorCriticPolicy, self).__init__(sess, ob_space, ac_space,
                                                n_env, n_steps, n_batch, reuse=reuse, scale=scale)
...
class FeedForwardPolicy(ActorCriticPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, seedy=None, layers=None, net_arch=None,
                 act_fun=tf.tanh, cnn_extractor=nature_cnn, feature_extraction="cnn", **kwargs):
        super(FeedForwardPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=reuse, seedy=seedy,
                                                scale=(feature_extraction == "cnn"))
...
class MlpPolicy(FeedForwardPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, seedy=None, **_kwargs):
        super(MlpPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse, seedy=seedy,
...
def register_policy(name, policy):
    ...
    if sub_class is None:
        raise ValueError(
            "Error: the policy {} is not of any known subclasses of BasePolicy!".format(policy))
